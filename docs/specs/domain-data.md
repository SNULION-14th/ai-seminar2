...
> Status: **Approved** (2026-09-24) · 관련: [architecture.md](architecture.md)
...
- [ ] 구간 거리 합계가 633km ± 5km 이내이다 (단위 테스트)
- [ ] 모든 인증센터의 `kmFromStart`가 오름차순이다 (단위 테스트)
- [ ] 모든 인증센터 좌표가 한반도 범위(위도 34-38.5, 경도 126-129.5) 안에 있고, 인접한 센터 간 직선거리가 누적거리 차이보다 짧다 (단위 테스트, 초안 데이터 sanity check)
- [ ] `snapToRoute`가 경로 위 좌표에서 오차 0.5km 이내를 반환한다 (단위 테스트)
